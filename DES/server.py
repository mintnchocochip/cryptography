...
def bits_to_hex(bits):
    # bits -> hex using int(join(bits),2)
    if not bits:
        return ""
    # keep leading zeros by padding to nibble boundary
    pad = (-len(bits)) % 4
    if pad:
        bits = ([0] * pad) + bits
    out = []
    for i in range(0, len(bits), 4):
        out.append(format(bits_to_int(bits[i : i + 4]), "x"))
    return "".join(out)


def transmit(key, cipher_hex):
    server = socket.socket()
    server.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    server.bind(("127.0.0.1", 65432))
    server.listen(1)
    conn, addr = server.accept()
    conn.send((key + ":" + cipher_hex).encode())
    conn.close()
    server.close()
