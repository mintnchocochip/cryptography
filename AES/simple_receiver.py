# simple_receiver.py
# Simplified AES-128 Decryption using 4x4 Matrices

SBOX = [
    
]

INV_SBOX = [
    
]

RCON = [0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36]

def expand_key(key_bytes):
    words = []
    for i in range(0, 16, 4):
        words.append(list(key_bytes[i:i+4])) 
        
    for i in range(4, 44):
        temp = words[i - 1][:]
        if i % 4 == 0:
            temp = temp[1:] + temp[:1]          
            temp = [SBOX[b] for b in temp]      
            temp[0] ^= RCON[(i // 4) - 1]       
        new_word = [words[i - 4][j] ^ temp[j] for j in range(4)]
        words.append(new_word)
        
    matrices = []
    for i in range(11):
        matrix = [[0]*4 for _ in range(4)]
        for c in range(4):
            for r in range(4):
                matrix[r][c] = words[i * 4 + c][r]
        matrices.append(matrix)
    return matrices

def bytes_to_matrix(data):
    matrix = [[0]*4 for _ in range(4)]
    for i in range(16): matrix[i % 4][i // 4] = data[i]
    return matrix

def matrix_to_bytes(matrix):
    return [matrix[r][c] for c in range(4) for r in range(4)]

def add_round_key(state, key_matrix):
    for r in range(4):
        for c in range(4):
            state[r][c] ^= key_matrix[r][c]
    return state

def inv_sub_bytes(state):
    for r in range(4):
        for c in range(4):
            state[r][c] = INV_SBOX[state[r][c]]
    return state

def inv_shift_rows(state):
    state[0] = state[0]
    state[1] = state[1][-1:] + state[1][:-1]
    state[2] = state[2][-2:] + state[2][:-2]
    state[3] = state[3][-3:] + state[3][:-3]
    return state

def xtime(a):
    return (((a << 1) ^ 0x1B) & 0xFF) if (a & 0x80) else (a << 1)

def multiply(a, b):
    res = 0
    while b:
        if b & 1: res ^= a
        a = xtime(a)
        b >>= 1
    return res

def inv_mix_columns(state):
    for c in range(4):
        s0, s1, s2, s3 = state[0][c], state[1][c], state[2][c], state[3][c]
        state[0][c] = multiply(s0, 14) ^ multiply(s1, 11) ^ multiply(s2, 13) ^ multiply(s3, 9)
        state[1][c] = multiply(s0, 9) ^ multiply(s1, 14) ^ multiply(s2, 11) ^ multiply(s3, 13)
        state[2][c] = multiply(s0, 13) ^ multiply(s1, 9) ^ multiply(s2, 14) ^ multiply(s3, 11)
        state[3][c] = multiply(s0, 11) ^ multiply(s1, 13) ^ multiply(s2, 9) ^ multiply(s3, 14)
    return state

def decrypt(ciphertext_bytes, key_bytes):
    round_keys_matrices = expand_key(key_bytes)
    state = bytes_to_matrix(ciphertext_bytes)
    
    state = add_round_key(state, round_keys_matrices[10])
    
    for i in range(9, 0, -1):
        state = inv_shift_rows(state)
        state = inv_sub_bytes(state)
        state = add_round_key(state, round_keys_matrices[i])
        state = inv_mix_columns(state)
        
    state = inv_shift_rows(state)
    state = inv_sub_bytes(state)
    state = add_round_key(state, round_keys_matrices[0])
    
    return matrix_to_bytes(state)

if __name__ == '__main__':
    key = b'Thats my Kung Fu'
    cipher = bytes.fromhex('29c3505f571420f6402299b31a02d73a')
    plain = decrypt(cipher, key)
    print('Decrypted (Text):', bytes(plain).decode())
