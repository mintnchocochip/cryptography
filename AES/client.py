...
# Single-block demo: no PKCS#7 padding is used.
...
def decrypt_message(cipher_hex: str, key_hex: str) -> Tuple[str, List[str]]:
    # Single-block AES-128 only (16 bytes ciphertext, 16 bytes key)
    key_bytes = bytes_from_hex(key_hex)
    round_keys = expand_key(key_bytes)

    if len(cipher_hex) != 32:
        raise ValueError("Single-block mode only: cipher text hex must be exactly 32 hex chars (16 bytes)")

    logs: List[str] = []
    block_bytes = bytes_from_hex(cipher_hex)
    plain_block, block_logs = decrypt_block(block_bytes, round_keys)
    logs.extend([f"Block 0 - {entry}" for entry in block_logs])

    plaintext = bytes(plain_block).decode(errors="ignore")
